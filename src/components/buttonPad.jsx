...
class ButtonPad extends Component {
    ...
    render(){
        ...
        return (<React.Fragment>
            ...
            <center>
            <div className="row" >
            {this.state.buttons.map(button => (button.id <=4) ?
                <MusicButton id={button.id} key={button.id} value={button.value}
                onClickButton={this.handleMusicButtonClicked} color={colorButton}/>
                : "")}
            </div>
            <div className="row" >
            {this.state.buttons.map(button => (button.id > 4 && button.id <= 8) ?
                <MusicButton id={button.id} key={button.id} value={button.value}
                onClickButton={this.handleMusicButtonClicked} color={colorButton}/>
                : "")}
            </div>
            <div className="row" >
            {this.state.buttons.map(button => (button.id > 8 && button.id <= 12) ?
                <MusicButton id={button.id} key={button.id} value={button.value}
                onClickButton={this.handleMusicButtonClicked} color={colorButton}/>
                : "")}
            </div>
